...
//include ('verify_function.php');
$uin = $_POST['id'];
//$userid = $_POST['transid'];
...
if($result) {
    ...
    while($row = mysqli_fetch_array($result)) {
		...
		if($temp_id!==$uin && $par_check==1){
			$sql_remark="UPDATE studentbiodata SET qualification_status='Pending', modified_by='$userid', date_modified='$dat' WHERE uin='$uin'";
		...
		if(trim($row['windexno'])!==''){
			...
			$sql_other_results="SELECT indexnum, surname, firstname, middlename, gradclass, graddate, entrylevel, cla, exam_date, progname FROM studentbiodata
			INNER JOIN tbl_graduate ON tbl_graduate.indexno=studentbiodata.indexno
			INNER JOIN tbl_other_results ON tbl_graduate.indexno=indexnum AND trans_id='$uin'
			INNER JOIN programme ON progid=sprogid";
			$rs_other_results=mysqli_query($conn, $sql_other_results);
			...
			if($rs_other_results->num_rows>0){
				...
				/*$sql_other_verified="SELECT * FROM tbl_other_verified
				INNER JOIN tbl_other_results ON uin=trans_id AND tbl_other_results.indexnum=tbl_other_verified.indexnum AND tbl_other_results.exam_date=tbl_other_verified.exam_date AND tbl_other_results.prog=tbl_other_verified.prog AND tbl_other_results.cla=tbl_other_verified.class";
				*/
