...
if($sid != '' && $name != '' && $prog != '' && $graddate != '') {
	...
	$query .= "SELECT uin, studentbiodata.indexno, firstname, middlename, surname, certno, progname, gradclass, graddate, qualification_status FROM studentbiodata INNER JOIN tbl_graduate ON tbl_graduate.indexno = studentbiodata.indexno LEFT JOIN  programme  ON sprogid  = progid WHERE concat_ws(' ', firstname, middlename, surname) LIKE '%{$name}%' AND sprogid='$prog' AND graddate='$graddate' AND (uin='$sid' OR studentbiodata.indexno='$sid')";
} 
